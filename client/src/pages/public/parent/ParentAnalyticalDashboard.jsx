...
import {
    ...
    useTheme, Divider, Paper, Stack, AppBar, Toolbar
} from '@mui/material';
import {
    BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer,
    PieChart, Pie, Cell, LineChart, Line, AreaChart, Area
} from 'recharts';
import { getStudentAnalytics, getParentStudentSnapshot } from '../../../services/api';
...
const ParentAnalyticalDashboard = () => {
    ...
    const fetchData = async () => {
        try {
            const [snapRes, analyticsRes] = await Promise.all([
                getParentStudentSnapshot(parent.student_id),
                getStudentAnalytics(parent.student_id)
            ]);
            setStudentInfo(snapRes.data);
    ...
    return (
        <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: '#0a192f', overflow: 'hidden' }}>
            ...
            <Box sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden', position: 'relative' }}>
                ...
                <AppBar position="static" elevation={0} sx={{ bgcolor: 'transparent', borderBottom: '1px solid rgba(255,255,255,0.05)', backdropFilter: 'blur(10px)', zIndex: 10 }}>
                    <Toolbar sx={{ justifyContent: 'space-between', minHeight: '70px !important', px: { md: 4 } }}>
                        <Box>
                            <Typography variant="h6" sx={{ color: '#ffffff', fontWeight: 700 }}>
                                Ward Analytical Dashboard
                            </Typography>
                ...
                <Box sx={{ flexGrow: 1, overflowY: 'auto', p: { xs: 2, md: 5 }, position: 'relative', zIndex: 1 }}>
                    <Container maxWidth="xl" sx={{ pb: 4 }}>
                        <Box sx={{ mb: 6 }}>
                            <Typography variant="h3" sx={{ color: '#ccd6f6', fontWeight: 900, mb: 1, letterSpacing: -0.5 }}>
                                Learning <span style={{ color: '#64ffda' }}>Analytics</span>
                            </Typography>
                            <Stack direction="row" spacing={2} alignItems="center">
                                ...
                                <Typography variant="body1" sx={{ color: '#8892b0', fontWeight: 500 }}>
                                    Performance overview for {studentInfo?.name} (Sem {studentInfo?.current_sem})
                                </Typography>
                            </Stack>
                        </Box>
                        ...
                        <Grid container spacing={4} sx={{ mb: 6 }}>
                            <Grid item xs={12} sm={6} md={3}>
                                <StatCard title="Avg Internal" value={summary?.avg_internal || 0} unit="pts" icon={<AssessmentIcon />} color="#64ffda" subtitle="Institutional score" />
                            </Grid>
                            <Grid item xs={12} sm={6} md={3}>
                                <StatCard title="Avg External" value={summary?.avg_external || 0} unit="pts" icon={<AssessmentIcon />} color="#ff7b72" subtitle="Board examinations" />
                            </Grid>
                            <Grid item xs={12} sm={6} md={3}>
                            ...
                            <Grid item xs={12} sm={6} md={3}>
                                <StatCard title="Study Hours" value={summary?.avg_study_hours || 0} unit="h" icon={<AccessTimeIcon />} color="#fbbf24" subtitle="Weekly study commitment" />
                            </Grid>
                        </Grid>

                        <Grid container spacing={4}>
                            <Grid item xs={12}>
                                <ModernChartContainer title="Subject Performance Comparison" icon={<TrendingUpIcon />} color="#64ffda">
...
export default ParentAnalyticalDashboard;

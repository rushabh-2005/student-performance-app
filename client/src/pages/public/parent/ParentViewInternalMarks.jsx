...
import { getParentStudentSnapshot, getSubjectsBySem, getStudentInternals } from '../../../services/api';
...
const ParentViewInternalMarks = () => {
    ...
    const fetchData = async (studentId) => {
        try {
            ...
            const resRes = await getStudentInternals(studentId);
                        ...
                        {loading ? <CircularProgress sx={{ display: 'block', mx: 'auto', mt: 10 }} /> : (
                            <Paper elevation={0} sx={{ p: 4, borderRadius: 4, border: '1px solid #edf1f7', bgcolor: '#fff' }}>
                                <Box sx={{ textAlign: 'center', mb: 4 }}>
                                    ...
                                    <Typography variant="subtitle1" color="textSecondary">{studentData?.name} | {studentData?.current_sem} | Institutional Assessment</Typography>
                                </Box>
                                <Divider sx={{ mb: 4 }} />
                                {!marksData ? (
                                    <Box padding={5} textAlign="center">
                                        <Typography variant="h6" color="textSecondary">Internal marks for this semester have not been published yet.</Typography>
                                    </Box>
                                ) : (
                                    <TableContainer>
                                        <Table>
                                            ...
                                            <TableBody>
                                                    ...
                                                    return (
                                                        <TableRow key={sub.id} hover>
                                                            ...
                                                            <TableCell align="center">
                                                                <Chip label={mark >= 40 ? "Satisfactory" : "Needs Improvement"} color={mark >= 40 ? "success" : "warning"} size="small" variant="outlined" sx={{ fontWeight: 'bold' }} />
                                                            </TableCell>
                                                        </TableRow>
                                                    );
                                                })}
                                                ...
                                                <TableRow>
                                                    <TableCell align="right"><Typography variant="subtitle1" fontWeight="bold">Total Score</Typography></TableCell>
                                                    <TableCell align="center">
...
export default ParentViewInternalMarks;
